<template>
	<div>
		...
		<!-- <div class="icon-font icon-kaquan" style="width:100px;height:100px"></div> -->
	</div>
</template>
...
<script>
	...
	export default {
		...
		mounted() {
			...
			//渲染数据
			this.addressItem = this.myAddress[this.$route.params.index]
			let addressType = this.addressItem.type
			// console.log(addressType)
			this.addressTypes.map((item, index) => {
				if (item === addressType) {
					this.currentTypeIndex = index
				}
			})
			this.value = this.addressItem.default_
			// console.log(index_)
		},
		computed: {
			...mapState(['myAddress']),
		...
		methods: {
			save() {
				...
				//触发action  更新地址信息
				// console.log(this.addressItem.default_)
				this.$store.dispatch('updateAddressInfo', {addressIndex: this.$route.params.index, addressItem: this.addressItem})
				setTimeout(() => {
